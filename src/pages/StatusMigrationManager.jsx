...
export default function StatusMigrationManager() {
  ...
  const activateNewBoardMutation = useMutation({
    mutationFn: async () => {
      // Step 1: Create the new master board directly
      const newBoardConfig = {
        organization_id: organization.id,
        board_type: "master",
        board_name: "All Proposals - New",
        is_master_board: false, // Start as inactive
      ...
      const newBoard = await base44.entities.KanbanConfig.create(newBoardConfig);

      // Step 2: Find the old master board
      const boards = await base44.entities.KanbanConfig.filter({ 
        organization_id: organization.id 
      });
      
      const oldMasterBoard = boards.find(b => 
        b.is_master_board === true
      );

      // Step 3: Activate new board and deactivate old board
