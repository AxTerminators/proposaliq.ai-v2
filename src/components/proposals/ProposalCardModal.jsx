...
export default function ProposalCardModal({ proposal: proposalProp, isOpen, onClose, organization, kanbanConfig, initialModalToOpen = null }) {
  ...
  // CRITICAL: Always call hooks unconditionally - pass empty strings if IDs are undefined
  ...
  const currentColumn = React.useMemo(() => {
    if (!kanbanConfig?.columns || !proposal) {
      console.log('[ProposalCardModal] currentColumn: no kanbanConfig or proposal');
      return null;
    }
    
    console.log('[ProposalCardModal] Finding column for proposal:', {
      proposalId: proposal.id,
      status: proposal.status,
      custom_workflow_stage_id: proposal.custom_workflow_stage_id,
      current_phase: proposal.current_phase,
      is_master_board: kanbanConfig?.is_master_board,
      columnsCount: kanbanConfig.columns.length
    });
    ...
    console.log('[ProposalCardModal] Found column:', found?.label || 'NONE', found?.id || 'N/A');
    return found;
  ...
  const handleChecklistItemClick = async (item) => {
    ...
    if (isNavigateAction(item.associated_action)) {
      ...
      if (actionConfig.path === 'ProposalStrategyConfigPage' || actionConfig.path === 'AIAssistedWriterPage') {
        console.log('[ProposalCardModal] 🚀 Navigating to:', actionConfig.path, 'with proposalId:', proposal.id);
        const url = `${createPageUrl(actionConfig.path)}?proposalId=${proposal.id}`;
        console.log('[ProposalCardModal] 🔗 Full URL:', url);
        window.location.href = url;
        return;
      }
